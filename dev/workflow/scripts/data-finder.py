...
def add_new_package(package, date):
    ...
    new_package_summary["verbatim_data_type"] = package.get("resources", [{}])[0].get(
        "resource_type"
    )
    ...
    new_package_summary["date_of_transfer_to_archive"] = package.get(
...
with open(args.metadata_path, "rt") as f:
    for line in f:
        ...
        if package.get(
            "metadata_created"
        ):  # using metadata_created as best estimate of when data were actually published on the DP (so far all packages have had this field)
            ...
            logger.debug(
                "found a package without 'metadata_created', 'date_of_transfer' or 'date_of_transfer_to_archive' field"
            )
            continue
        if args.date_last_checked <= transfer_date:
            logger.debug("found a new package!")
            if (
                not target_initiatives
                or package.get("organization", {}).get("title", None)
                in target_initiatives
            ):
    ...
    logger.info(
        f"Found {undated_package_counter} packages missing 'metadata_created', 'date_of_transfer' or 'date_of_transfer_to_archive' fields in their metadata."
    )
...
if len(new_assembly_packages) > 0:
    logger.info(
        f"Found {len(new_assembly_packages)} new packages with relevant data types."
    )
    sorted_filtered_summary = sort_output(new_assembly_packages)
    write_to_csv(sorted_filtered_summary, relevant_table_path)
else:
    logger.info(
        "No new packages with relevant data types found for target initiative/s."
    )
